...
python topic2_act/scripts/summarize_act_run.py \
  --run-dir topic2_act/outputs/act_calvin/a_only_smoke500_50ep \
...
python topic2_act/eval/run_bridge_fidelity.py \
  --checkpoint topic2_act/outputs/act_calvin/a_only_smoke500_50ep/lerobot_train/checkpoints/last \
  --dataset-root topic2_act/data/xiaoma26_calvin_lerobot/splitA_episodes_A_smoke500_canonical_v3 \
...
python topic2_act/eval/run_calvin_eval.py \
  ...
  --eval-log-dir topic2_act/outputs/calvin_eval/day7_real_act_direct_cameras \
  --cuda-device 0 \
...
python topic2_act/scripts/prepare_xiaoma_calvin_split.py \
  ...
  --output-dir topic2_act/data/splits/xiaoma26_calvin_lerobot \
  --download-split splitA \
  --metadata-splits splitA splitB splitC splitD \
  --smoke-count 500 \
  --seed 20260606 \
  --manifest topic2_act/data/xiaoma26_calvin_lerobot_manifest.json
...
root = Path("topic2_act/data/splits/xiaoma26_calvin_lerobot")
...
du -sh topic2_act/data/xiaoma26_calvin_lerobot \
...
python topic2_act/scripts/run_act_train.py \
  --dataset-root topic2_act/data/xiaoma26_calvin_lerobot/splitA \
  --episodes-file topic2_act/data/splits/xiaoma26_calvin_lerobot/episodes_A_full.json \
  --output-dir topic2_act/outputs/act_calvin_dataloader/splitA_prep_probe \
...
```bash
python topic2_act/scripts/summarize_act_run.py \
  --run-dir topic2_act/outputs/act_calvin_dataloader/splitA_prep_probe \
...
for spec in nw4_pf2_np nw8_pf4_p nw12_pf4_p nw16_pf4_p; do
  ...
  python topic2_act/scripts/run_act_train.py \
    --dataset-root topic2_act/data/xiaoma26_calvin_lerobot/splitA \
    --episodes-file topic2_act/data/splits/xiaoma26_calvin_lerobot/episodes_A_full.json \
    --output-dir topic2_act/outputs/act_calvin_dataloader/splitA_${spec}_2k_fix1 \
...
for spec in nw4_pf2_np nw8_pf4_p nw12_pf4_p nw16_pf4_p; do
  python topic2_act/scripts/summarize_act_run.py \
    --run-dir topic2_act/outputs/act_calvin_dataloader/splitA_${spec}_2k_fix1 \
...
mkdir -p .secrets
chmod 700 .secrets
export NETRC=.secrets/wandb_netrc
umask 077
...
python topic2_act/scripts/run_act_train.py \
  --dataset-root topic2_act/data/xiaoma26_calvin_lerobot/splitA \
  --episodes-file topic2_act/data/splits/xiaoma26_calvin_lerobot/episodes_A_smoke500.json \
  --output-dir topic2_act/outputs/act_calvin/a_only_smoke500_dryrun \
...
python topic2_act/scripts/run_act_train.py \
  --dataset-root topic2_act/data/xiaoma26_calvin_lerobot/splitA \
  --episodes-file topic2_act/data/splits/xiaoma26_calvin_lerobot/episodes_A_smoke500.json \
  --output-dir topic2_act/outputs/act_calvin/a_only_smoke500_5ep \
...
```bash
python topic2_act/scripts/summarize_act_run.py \
  --run-dir topic2_act/outputs/act_calvin/a_only_smoke500_5ep \
...
python topic2_act/scripts/run_act_train.py \
  --dataset-root topic2_act/data/xiaoma26_calvin_lerobot/splitA \
  --episodes-file topic2_act/data/splits/xiaoma26_calvin_lerobot/episodes_A_smoke500.json \
  --output-dir topic2_act/outputs/act_calvin/a_only_smoke500_50ep \
...
```bash
cd topic2_act/calvin_official/dataset
sed -i 's/\r$//' download_data.sh

bash download_data.sh debug \
  2>&1 | tee logs/Day5/day5_fix_calvin_download_debug.log

ls -lah topic2_act/calvin_official/dataset \
  2>&1 | tee logs/Day5/day5_fix_calvin_dataset_listing.log
```

Acceptance: the dataset root is
`topic2_act/calvin_official/dataset/calvin_debug_dataset/`.
...
python topic2_act/eval/verify_calvin_eval_env.py \
  --calvin-root topic2_act/calvin_official \
  --cuda-device 0 \
  --egl-diagnostics-dir logs/Day5/egl_diag_verify \
...
```bash
cd topic2_act/calvin_official/calvin_env/egl_check
rm -f EGL_options.o
bash build.sh \
  2>&1 | tee logs/Day5/day5_egl_deep_calvin_build.log

./EGL_options.o \
  > logs/Day5/day5_egl_deep_calvin_egl_stdout.log \
  2> logs/Day5/day5_egl_deep_calvin_egl_stderr.log || true

cat logs/Day5/day5_egl_deep_calvin_egl_stderr.log
cat logs/Day5/day5_egl_deep_calvin_egl_stdout.log
```
...
DATASET=topic2_act/calvin_official/dataset/calvin_debug_dataset
CKPT=$(find topic2_act/outputs/act_calvin/a_only_smoke500_50ep/lerobot_train/checkpoints \
...
python topic2_act/eval/run_calvin_eval.py \
  ...
  --checkpoint "$CKPT" \
  --eval-log-dir topic2_act/outputs/calvin_eval/day5_zero_action_debug_direct_nocamera_gripperfix \
  --cuda-device 0 \
  --num-sequences 1 \
  --ep-len 2 \
  --egl-policy direct \
...
python topic2_act/scripts/probe_calvin_dataset.py \
  ...
  --endpoint https://hf-mirror.com \
  --revision main \
  --local-dir topic2_act/data/calvin_task_ABC_D_probe \
  --max-meta-files 50 \
